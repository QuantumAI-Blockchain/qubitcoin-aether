// wQBC — Wrapped Qubitcoin SPL Token on Solana
//
// This Anchor program implements the wQBC SPL token on Solana.
// QBC is locked on the Qubitcoin chain → bridge operator mints wQBC on Solana.
// User burns wQBC on Solana → bridge operator unlocks QBC on Qubitcoin chain.
//
// Features:
//   - SPL Token 2022 compatible
//   - Bridge operator authority for mint/burn
//   - 0.1% bridge fee (10 bps)
...
declare_id!("QBCwrapped111111111111111111111111111111111");

/// Bridge fee in basis points (10 = 0.1%).
const BRIDGE_FEE_BPS: u64 = 10;
...
pub mod wqbc {
    ...
    /// Initialize the wQBC bridge state.
    ...
    /// Bridge operator mints wQBC when QBC is locked on the QBC chain.
    pub fn bridge_mint(
        ...
        let state = &mut ctx.accounts.bridge_state;
        require!(!state.paused, WQBCError::Paused);
        require!(amount > 0, WQBCError::ZeroAmount);

        // Check replay protection
        let receipt = &mut ctx.accounts.tx_receipt;
        require!(!receipt.processed, WQBCError::AlreadyProcessed);
    ...
    /// User burns wQBC to redeem native QBC on the Qubitcoin chain.
    pub fn bridge_burn(
    ...
    /// Emergency pause — only bridge authority.
    pub fn pause(ctx: Context<AdminAction>) -> Result<()> {
        ctx.accounts.bridge_state.paused = true;
        Ok(())
    }

    /// Unpause — only bridge authority.
