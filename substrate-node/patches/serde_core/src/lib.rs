...
// Keep the macro definition available (serde crate may invoke it).
#[macro_use]
mod crate_root;
#[macro_use]
mod macros;

// ─── Inlined crate_root!() expansion ───────────────────────────────────────
// WASM FIX: By inlining the macro expansion, all module declarations (de, ser,
// format, private, std_error) are at the crate root level. This ensures the
// Rust compiler injects the implicit prelude into each module, providing
// built-in macros (stringify!, concat!, format_args!, derive, etc.) that are
// NOT available in modules declared inside macro expansions on
// wasm32-unknown-unknown with rustc 1.93+.

/// A facade around all the types we need from the `std`, `core`, and `alloc`
/// crates. This avoids elaborate import wrangling having to happen in every
/// module.
mod lib {
    // Renamed from `core` to `_facade` to avoid shadowing the extern `core`
    // crate on wasm32-unknown-unknown targets. The name `core` caused `::core`
    // inside the module to resolve to itself rather than the extern crate.
    mod _facade {
        #[cfg(not(feature = "std"))]
        pub use ::core::*;
        #[cfg(feature = "std")]
    ...
    pub use self::_facade::option::Option::{self, None, Some};
    pub use self::_facade::result::Result::{self, Err, Ok};
    pub use self::_facade::iter::Iterator;
    pub use self::_facade::clone::Clone;
    pub use self::_facade::marker::{Copy, Send, Sized, Sync, Unpin};
    pub use self::_facade::convert::{From, Into};
    pub use self::_facade::default::Default;
    pub use self::_facade::cmp::{Eq, Ord, PartialEq, PartialOrd};
    pub use self::_facade::ops::{Drop, Fn, FnMut, FnOnce};
    pub use self::_facade::{f32, f64};
    pub use self::_facade::{iter, num, str};

    #[cfg(any(feature = "std", feature = "alloc"))]
    pub use self::_facade::{cmp, mem};

    pub use self::_facade::cell::{Cell, RefCell};
    pub use self::_facade::cmp::Reverse;
    pub use self::_facade::fmt::{self, Debug, Display, Write as FmtWrite};
    pub use self::_facade::marker::PhantomData;
    pub use self::_facade::num::Wrapping;
    pub use self::_facade::ops::{Bound, Range, RangeFrom, RangeInclusive, RangeTo};
    pub use self::_facade::result;
    pub use self::_facade::time::Duration;
    ...
    pub use self::_facade::num::Saturating;
}
