...
export function useVaultState() {
  return useQuery<VaultState>({
    ...
    queryFn: async (): Promise<VaultState> => {
      ...
      return {
        qbcLocked: result.qbcLocked ?? 0,
        qusdLocked: result.qusdLocked ?? 0,
        wqbcByChain: (result.wqbcByChain ?? {}) as VaultState["wqbcByChain"],
        wqusdByChain: (result.wqusdByChain ?? {}) as VaultState["wqusdByChain"],
        totalWqbc: result.totalWqbc ?? 0,
        totalWqusd: result.totalWqusd ?? 0,
        backingRatioQbc: (result.backingRatioQbc ?? 0) as VaultState["backingRatioQbc"],
        backingRatioQusd: (result.backingRatioQusd ?? 0) as VaultState["backingRatioQusd"],
        vaultAddrQbc: result.vaultAddrQbc ?? "",
        vaultAddrQusd: result.vaultAddrQusd ?? "",
        dailyWrapVol: result.dailyWrapVol ?? 0,
        dailyUnwrapVol: result.dailyUnwrapVol ?? 0,
