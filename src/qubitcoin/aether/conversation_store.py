...
class ConversationStore:
    ...
    def create_session(
        ...
        try:
            with self.db.get_session() as session:
                session.execute(text("""INSERT INTO conversation_sessions
    ...
    def update_session(
        ...
        try:
            with self.db.get_session() as session:
                ...
                session.execute(
                    text(f"UPDATE conversation_sessions SET {', '.join(updates)} WHERE session_id = :sid"),
                    params,
                )
                session.commit()
        except Exception as e:
    ...
    def archive_session(self, session_id: str) -> None:
        ...
        try:
            with self.db.get_session() as session:
                session.execute(
                    text("UPDATE conversation_sessions SET status = 'archived' WHERE session_id = :sid"),
                    {'sid': session_id},
                )
                session.commit()
        except Exception as e:
    ...
    def recall(self, user_id: str, key: str) -> Optional[str]:
        ...
        try:
            with self.db.get_session() as session:
                row = session.execute(
                    text("SELECT memory_value FROM user_memory WHERE user_id = :uid AND memory_key = :key"),
                    {'uid': user_id, 'key': key},
                ).fetchone()
        ...
        try:
            with self.db.get_session() as session:
                rows = session.execute(
                    text("SELECT memory_key, memory_value FROM user_memory WHERE user_id = :uid ORDER BY updated_at DESC"),
                    {'uid': user_id},
                ).fetchall()
                for row in rows:
    ...
    def forget(self, user_id: str, key: str) -> None:
        ...
        try:
            with self.db.get_session() as session:
                session.execute(
                    text("DELETE FROM user_memory WHERE user_id = :uid AND memory_key = :key"),
                    {'uid': user_id, 'key': key},
                )
                session.commit()
        except Exception as e:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            with self.db.get_session() as session:
                # Total sessions
                row = session.execute(text("SELECT COUNT(*) FROM conversation_sessions")).fetchone()
                stats['total_sessions'] = row[0] if row else 0

                # Active sessions
                row = session.execute(
                    text("SELECT COUNT(*) FROM conversation_sessions WHERE status = 'active'")
                ).fetchone()
                stats['active_sessions'] = row[0] if row else 0

                # Total messages
                row = session.execute(text("SELECT COUNT(*) FROM conversation_messages")).fetchone()
                stats['total_messages'] = row[0] if row else 0

                # Unique users
                row = session.execute(
                    text("SELECT COUNT(DISTINCT user_id) FROM conversation_sessions")
                ).fetchone()
                stats['unique_users'] = row[0] if row else 0

                # User memories
                row = session.execute(text("SELECT COUNT(*) FROM user_memory")).fetchone()
                stats['total_memories'] = row[0] if row else 0

                # Insights
                row = session.execute(text("SELECT COUNT(*) FROM conversation_insights")).fetchone()
