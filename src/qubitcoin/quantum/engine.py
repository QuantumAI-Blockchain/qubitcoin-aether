...
class QuantumEngine:
    ...
    def _initialize_backend(self):
        ...
        if Config.USE_LOCAL_ESTIMATOR:
            ...
            logger.info("⚛️  Quantum Engine: Local StatevectorEstimator (exact)")
            ...
            try:
                ...
                self.estimator = StatevectorEstimator()
                logger.info("⚛️  Quantum Engine: Noisy AerSimulator")
            except ImportError:
    ...
    def generate_hamiltonian(self, num_qubits: int = 4, seed: Optional[int] = None) -> List[Tuple[str, float]]:
        ...
        Returns:
            List of (pauli_string, coefficient) tuples - CORRECT ORDER FOR QISKIT!
        """
        if seed is not None:
            np.random.seed(seed)
        ...
        for _ in range(num_terms):
            ...
            # IMPORTANT: Qiskit expects (pauli_string, coefficient) order!
    ...
    def compute_energy(self, params: np.ndarray, hamiltonian: List[Tuple[str, float]], 
        ...
        # Create observable - hamiltonian is already in correct (pauli, coeff) format
